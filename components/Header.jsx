...
function Header() {
  return (
    <div className='w-11/12 max-w-3xl text-center mx-auto h-screen flex flex-col items-center justify-center gap-4'>
      ...
      >
        <Image src={assets.profile_img} alt='profile-pic' className='rounded-full w-32 ' />
      </motion.div>
      <motion.h3
        initial={{ y: -20, opacity: 0 }}
        whileInView={{ y: 0, opacity: 1 }}
        transition={{ duration: 0.6, delay: 0.3 }}
        className='flex items-end gap-2 text-xl md:text-2xl mb-3 '>Hi! I'm William Mark</motion.h3>
      <Image src={assets.hand_icon} alt='hand-icon' className='w-6' />
      <motion.h1
        initial={{ y: -30, opacity: 0 }}
        whileInView={{ y: 0, opacity: 1 }}
        transition={{ duration: 0.8, delay: 0.5 }}
        className='text-3xl sm:text-6xl lg:text-[66px] '>Frontend web developer based in London.</motion.h1>
      <motion.p
        initial={{ opacity: 0 }}
